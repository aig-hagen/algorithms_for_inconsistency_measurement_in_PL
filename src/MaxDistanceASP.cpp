...
std::string handle_formulas_in_kb_d_max(Kb& kb){
    ...
    for (int i = 0; i < formulas.size(); i++){
        ...
        pl_to_ASP_d_max(formulas.at(i), formula_name, curr_formula_rules, formulas.size());
...
void pl_to_ASP_d_max(Formula& formula, std::string formula_name, std::string& rules, int num_formulas){
    if(formula.IsAtom()){
    ...
    if(formula.IsNegation()){
        ...
        pl_to_ASP_d_max(formula_without_negation, new_formula_name, rules, num_formulas);
    }
    ...
    if(formula.IsConjunction()){
        ...
        for(auto conjunct : conjuncts){
            ...
            pl_to_ASP_d_max(conjunct, new_formula_name, rules, num_formulas);
        }
    }
    ...
    if(formula.IsDisjunction()){
        ...
        for(auto disjunct : disjuncts){
            ...
            pl_to_ASP_d_max(disjunct, new_formula_name, rules, num_formulas);
        }
    }
    ...
    if(formula.IsImplication()){
        ...
        pl_to_ASP_d_max(disj, formula_name, rules, num_formulas);
    }
    ...
    if(formula.IsEquivalence()){
        ...
        pl_to_ASP_d_max(conj, formula_name, rules, num_formulas);
    }
...
int max_distance_measure(Kb& kb){
    ...
    // kbMembers hinzufügen
    // -> + Regeln, den Namen [und den Index] auszulesen
    // program += KB_MEMBER + "(F):-" + KB_MEMBER + "(F,_).";
    program += handle_formulas_in_kb_d_max(kb);
    ...
    program += DIFF + "(1..X):-X=#count{A:" + ATOM + "(A)}.";
    program += DALAL + "(X):-" + DIFF + "(X)," + INTERPRETATION + "(I),X<=#count{A:" + ATOM + "(A)," + TRUTH_VALUE_PREDICATE_INTERPRETATION + "(A,I,T),not " + TRUTH_VALUE_PREDICATE_INTERPRETATION + "(A," + std::to_string(numFormulas) +  ",T)}.";

    // Maximization
    // program += DALAL_MAX + "(X):-" + DALAL + "(I1," + std::to_string(numFormulas) + ",X),not " + DALAL + "(I2," + std::to_string(numFormulas) + ",Y),Y>X.";
    // program += DALAL_MAX + "(X):-" + DALAL + "(I1," + std::to_string(numFormulas) + ",X),not " + DALAL + "(I2," + std::to_string(numFormulas) + ",Y),Y>X," + INTERPRETATION + "(I1)," + INTERPRETATION + "(I2).";
    // program += DALAL_MAX + "(X):-X=#max{Y:" + DALAL + "(I," + std::to_string(numFormulas) + ",Y)," + INTERPRETATION + "(I)},X>=0.";

    // Minimization:
    // program += "#minimize{X:" + DALAL_MAX + "(X)}.";
    program += "#minimize{1,X:" + DALAL + "(X)}.";
